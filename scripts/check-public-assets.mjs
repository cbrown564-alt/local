 ...
 *
 *   node scripts/check-public-assets.mjs            # guard, quiet summary
 *   node scripts/check-public-assets.mjs --orphans  # list unreferenced assets
 */
...
const ALLOWED_TOP_LEVEL = new Set([
  ...
  "images", // concept, comparison and place imagery
  "videos", // demo clips and journey films
  "favicon.svg",
  "robots.txt",
]);
  ...
  if (showOrphans) {
    ...
    console.log(
      "\nAn unused file is not a problem in itself. It is a problem when a" +
        ...
        "\nresearch/concept-reviews/image-provenance.md.\n",
    );
  } else {
    console.log(
      `Public asset check passed. Note: ${orphans.length} held asset(s) unreferenced from src/ — ` +
        `run \`node scripts/check-public-assets.mjs --orphans\` to list them.`,
    );
  }
} else {
