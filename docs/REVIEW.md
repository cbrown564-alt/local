...
- [x] **GitHub repo was public** while `src/site/data/businesses.json` and
  `research/pipeline/verifications.json` carry 379 real named businesses with phones,
  ...
  every commit with `tools/purge-sensitive-git-history.sh`; current copies
...
- [x] **The printed one-sheet QR still encoded the old domain after the
  ...
  `tools/print/print-onesheet.mjs` but not the asset it writes, so
  `public/media/concepts/hotel-enniskeen/hotel-enniskeen-onesheet-qr.svg` stayed byte-identical to a QR
  ...
  docs/MEDIA_CAPTURE.md. The PDF under gitignored `.scratch/print/pdf/` still predates the
...
- [x] **The Dundrum Inn transformation page showed the wrong "after".** Closed
  ...
  still and clip (`/media/concepts/dundrum-inn/dundrum-inn-after.jpg`,
  `/media/concepts/dundrum-inn/dundrum-inn-after.mp4`) so the case study matches the current
...
- [ ] **Newcastle Chamber's CC BY-SA credit sits below the phone fold.** The
  ...
  page and in `public/media/place/ATTRIBUTION.md` — so this is a standard
  question, not a breach: decide whether `docs/CONCEPT_DESIGN_REVIEW.md` should
...
- Concept visual quality — each carries the subject's identity, never the
  studio's, exactly as docs/DESIGN.md prescribes.
