...
spent against an incomplete defect list. The second repair and its independent
re-review are now complete. All five again returned **Revise** and remain
internal `noindex` routes.
...
This is the implementation record for the source reviewed below. `pnpm build`,
the 72-case concept audit and all 14 reviewed-concept journeys passed before
review, and a 28-route sweep at 1265×710 and a true 390×844 found zero
horizontal overflow, zero console errors and zero contrast failures. The
independent reviewers then found gaps in the assertions themselves, most
notably inherited `opacity`, focus-ring contrast against the surface outside a
control, and content hidden underneath the fixed disclosure.

#### Independent re-review of the reopened candidates — 25 July 2026

The five non-creator reviewers completed their measurements against the
fingerprints above. Three sessions ended on the account limit immediately after
their review records were written; their complete `Write` payloads and tool
results were recovered from Claude session
`51b9268c-055a-45a2-9531-72d6e0a91abb`. No score or finding below is inferred
from a partial chat summary.

| Concept | Previous | Reopened candidate | Verdict | Failed gates | Deciding defects |
|---|---:|---:|---|---|---|
| Mourne Cycles | 5.78 | **6.08** | Revise | subject proof, responsive/keyboard, readable/motion-safe | The hero proves a bicycle beside an unidentified fence, not the shop; the coal focus ring is 1.00:1 on the coal hero; three live texts faded with inherited `opacity` measure 3.30–3.93:1 |
| Newcastle Chamber | 5.88 | **6.68** | Revise | none | All gates pass, but Complete loop is 6.5 and the weighted score stays below 7.0: the advertised query `bikes` returns no result, category-only status copy says “for in”, and the phone fold loses the search and primary action |
| Kent Amusements | 5.43 | **6.38** | Revise | responsive/keyboard | The new dark focus token measures 1.00:1 on the strip and rail and 1.17:1 on the ticket action; the attractions rail is still covered by the fixed disclosure at ordinary scroll positions |
| Donard Veterinary | 6.53 | **6.33** | Revise | claims, responsive/keyboard | The concept substitutes PetsApp for the practice’s published VidiVet out-of-hours service; focus is 1.04–1.25:1 on the emergency surfaces; three appointments controls sit under the fixed disclosure at desktop load |
| Cúpla | 5.78 | **6.60** | Revise | responsive/keyboard | Focus is 1.14–1.54:1 on the dominant petrol surfaces; the phone disclosure covers the generated-image caption; nine Irish menu strings still lack `lang="ga"` |

The repairs did close the named second-round defects. Newcastle Chamber now
passes all six gates; Kent Amusements recovered from 5.43 to 6.38; Cúpla’s
visitor loop and text contrast pass; Mourne Cycles’ claims and asset permission
pass; Donard Veterinary’s contrast and stale-draft behaviour pass. The repeated
failure is measurement coverage, not evidence that those fixes did not land.

All five remain outside `publicTransformationSlugs`. The review does not
authorize another repair cycle. A further reopening needs an explicit owner
decision under §5. The smallest credible next work, if reopened, is:

1. add a portfolio-wide focus-indicator assertion that measures the painted
   ring against adjacent surfaces, then repair the shared and concept-specific
   focus rules;
2. change the generated-disclosure and fixed-banner assertions from rectangle
   checks to hit tests at representative scroll positions;
3. keep inherited `opacity` in the text-contrast probe;
4. repair the concept-specific truth and loop defects only after those shared
   checks fail correctly.

Coordinator verification after recovery: `pnpm build` passes with zero Astro
diagnostics and 73 generated pages. The corrected contrast probe makes
`pnpm test:reviewed-concepts` report **13/14 passed**, failing precisely on the
live Mourne Cycles text the reviewer measured (`Cycle to Work` 3.34:1 and
`Listed Cyclescheme retailer` 3.93:1; the qualifying hire-page note is reached
after that route is repaired). Disabled placeholder cells are excluded through
their runtime `aria-disabled="true"` state. This is an expected failing
regression assertion for a Revise candidate, not evidence that the review is
incomplete.
