...
Current presence and verification: `research/pipeline/verifications.json` → "Cupla",
...
- Current capture: `research/concepts/phase-q-rereview/evidence/cupla__desktop-1265x710.png`,
...
- Sources, asset credits and limitations: `research/pipeline/verifications.json` →
  "Cupla"; `public/media/place/ATTRIBUTION.md:13`.
...
in `docs/CONCEPT_DESIGN_REVIEW.md` as an implementation record rather than a verdict.
...
1. **Restore the menu route at 390.** `.cp-nav { display: none }`
   ...
   `tools/test/test-reviewed-concept-journeys.mjs`, because the 72-case audit
