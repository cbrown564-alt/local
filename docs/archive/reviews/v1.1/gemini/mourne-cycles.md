...
(re-computed by the reviewer via `node tools/check-concept-reviews.mjs --fingerprint mourne-cycles`; matches the repair candidate recorded in `docs/CONCEPT_DESIGN_REVIEW.md`)  
...
- Hero imagery: AI-generated visualisation, disclosed in an adjacent
  figcaption and in `public/media/place/ATTRIBUTION.md`.
...
- Desktop after, 1265×710:
  `research/concepts/phase-q-rereview/evidence/reviewer-mourne-cycles/v2-home-desktop.png`
...
- Phone first screen, 390×844:
  ...
  **Note on the supplied bundle:** the pre-captured images in
  `research/concepts/phase-q-rereview/evidence/` (`mourne-cycles__desktop-1265x710.png`,
...
- Sources, asset credits and limitations: `public/media/place/ATTRIBUTION.md`
  (hero visualisation entry); `src/site/data/transformation-details.ts` lines 731
...
verdict. Under the process rule at `docs/CONCEPT_DESIGN_REVIEW.md` §5, if a core
