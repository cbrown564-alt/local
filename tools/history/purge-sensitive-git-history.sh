...
PATHS_FILE="$ROOT/tools/purge-sensitive-git-history.paths"
BACKUP_DIR="$(mktemp -d)"
...
if [[ -d research/evidence ]]; then
  cp -R research/evidence "$EVIDENCE_BACKUP"
fi
mkdir -p "$DATA_BACKUP"
cp src/site/data/businesses.json "$DATA_BACKUP/"
cp research/pipeline/verifications.json "$DATA_BACKUP/"
...
if [[ -d "$EVIDENCE_BACKUP" ]]; then
  ...
  rm -rf research/evidence
  cp -R "$EVIDENCE_BACKUP" research/evidence
fi

echo "Recommitting operational datasets"
cp "$DATA_BACKUP/businesses.json" src/site/data/businesses.json
cp "$DATA_BACKUP/verifications.json" research/pipeline/verifications.json
git add src/site/data/businesses.json research/pipeline/verifications.json
...
echo "Verify with: git log --all --name-only -- public/media/concepts/castle-farm/castle-farm-box.jpg"
