...
//   node tools/capture/capture-concept-media.mjs <slug> [both|before|after] [both|video|still]
//   node tools/capture/capture-concept-media.mjs <slug> reel
//
...
// Journey films — the fairness rule (docs/MEDIA_CAPTURE.md, and the reason this mode
  ...
  "hotel-enniskeen": {
    ...
    segments: [
      ...
      },
      {
        ...
        captionStyle: "editorial",
        chapter: "03 · Dine",
        caption: "Menus readable on the page — dinner to afternoon tea, without a PDF download.",
        steps: [
          { action: "goto", url: "/concepts/hotel-enniskeen/dine/#panel-tea", settleMs: 1600 },
          { action: "hold", seconds: 3.2 },
          { action: "scroll", to: 0.18, unit: "page", durationMs: 900, seconds: 3.6 },
        ],
      },
      {
        id: "after-estate",
      ...
      },
      {
        ...
        captionStyle: "quiet",
        chapter: "05 · Explore",
        caption: "Routes into the Mournes, the coast and the links next door.",
        steps: [
          { action: "goto", url: "/concepts/hotel-enniskeen/things-to-do/", settleMs: 1200 },
          { action: "hold", seconds: 2.8 },
          { action: "scroll", to: 0.55, unit: "page", durationMs: 1100, seconds: 3.5 },
        ],
      },
      {
        id: "after-booking",
      ...
      },
      {
        ...
        before: { type: "image", path: "/media/concepts/hotel-enniskeen/hotel-enniskeen-before.jpg" },
        after: { type: "image", path: "/media/concepts/hotel-enniskeen/hotel-enniskeen-after.jpg" },
      },
      {
        id: "end-card",
        type: "card",
...
if (
  ...
  console.error(`Usage: node tools/capture/capture-concept-media.mjs <slug> [both|before|after|reel|journey] [both|video|still]\nKnown slugs: ${Object.keys(CONCEPTS).join(", ")}`);
  process.exit(1);
}
...
const conceptMediaDir = (conceptSlug) => {
  const dir = path.join(projectRoot, "public", "media", "concepts", conceptSlug);
  fs.mkdirSync(dir, { recursive: true });
  return dir;
};
// Landing captures and the flagship reel co-locate stills and videos under
// public/media/concepts/<slug>/. Companion stills use the same folder with a
// longer filename (e.g. hotel-enniskeen-dine-after.jpg).
const imageDir = conceptMediaDir(slug.includes("/") ? slug.split("/")[0] : slug);
const videoDir = imageDir;
...
// ---------------------------------------------------------------------------
// Demo choreography — keep total inside 9.5–10.5s (see docs/MEDIA_CAPTURE.md)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Journey films — see tools/lib/journeys.mjs for the step definitions and the
