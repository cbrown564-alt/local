...
//   node tools/capture/audit-journey.mjs bucks-head
//
// Output: .scratch/renders/<slug>-journey/<date>/
...
//
// The no-real-booking rule is enforced in tools/lib/journeys.mjs, not here:
...
if (!slug) {
  console.error("Usage: node tools/capture/audit-journey.mjs <slug>");
  process.exit(1);
}
...
const outDir = path.join(root, ".scratch", "renders", `${slug}-journey`, today);
...
const imageDir = path.resolve(root, "public", "media");
