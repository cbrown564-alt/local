...
(re-computed locally with `node scripts/check-concept-reviews.mjs --fingerprint kent-amusements` — matches the repair candidate recorded in `CONCEPT_DESIGN_REVIEW.md`)  
...
- Current capture: `research/verifications.json` (Facebook/Instagram-only
...
- **Evidence-bundle defect:** the supplied bundle in
  `research/concept-reviews/evidence/phase-q-rereview/` is not usable. Its own
...
1. **Invented seasonal hours.** `attractions.astro` lines 100–113 publish "10 am
   ...
   `scripts/test-reviewed-concept-journeys.mjs` that no time-of-day string
...
material-claim classes against `research/verifications.json`; computed contrast
