...
protocol in `docs/RESEARCH_METHOD.md` before any concept work, and the
...
`tools/pipeline/normalize-businesses.mjs` holds only five entries. Unmerged pairs found
this round:
...
`research/pipeline/verifications.json` as `tradingStatus: "Closed"` so the OpenStreetMap
...
These are **not** recorded in `research/pipeline/verifications.json`. A first-hand "yes,
