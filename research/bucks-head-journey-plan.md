...
- **Morning-of-build re-verification** — this site changes fast (ResDiary
  ...
  it in `research/verifications.json`. A pitch built on a stale critique is
  worse than no pitch.
- Same delivery gates as F1: domain resolves, request form delivers
  (REVIEW.md P1). Re-verify again the morning of any walk-in.
...
Screenshot every step to `research-renders/bucks-head-journey/<date>/`;
...
screen). Write the numbers into `research/verifications.json` — these
...
3. Keep the committed 1265×710 stills valid: if anything above the desktop
   fold changes, recapture per MEDIA_CAPTURE.md in the same commit.
...
Extend the F1 reel mode in `scripts/capture-concept-media.mjs` with a
`journey` subcommand:
...
- **No-real-booking rule (hard):** on the live site the script stops at the
  ...
  Encode the stop in the script and document it in MEDIA_CAPTURE.md.
- Outputs: `public/videos/bucks-head-journey-before.mp4` and `-after.mp4`
...
and ≤ 8 MB; capture-log rows and QA checklist additions in MEDIA_CAPTURE.md.
...
`research-renders/bucks-head-journey/`.
...
2. **Walk-in script** → `research/bucks-head-pitch.md`. This is a hometown,
...
3. Gates before anything is handed over: REVIEW.md P1 pair cleared;
...
Same-commit doc updates throughout: `research/verifications.json` (audit
numbers, scope decisions, stage change to **Contacted** only when the walk-in
happens), `PROSPECTS.md`, `PLAN.md` Milestone F, `MEDIA_CAPTURE.md`,
`DESIGN.md` if a new component ships.
