...
`PROSPECTS.md`; verification record in `research/verifications.json`.

The dated catalogue snapshot behind the 26 July numbers is
`research/painted-earth-catalogue-2026-07-26.json`.
...
Reason: `research/concept-reviews/image-provenance.md` records that reusing a
...
Taken from the shop's own marks, per the `DESIGN.md` rule that a concept
...
- **Build-day re-check completed 27 July 2026.** The central discovery claim
  ...
  narrowed and the change recorded here and in `research/verifications.json`.
...
- Publication still requires the five checks in `CONCEPT_DESIGN_REVIEW.md`, a
  Publish record in `research/concept-reviews/publication.json`, the slug in
