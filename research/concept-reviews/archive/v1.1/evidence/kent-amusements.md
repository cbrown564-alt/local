...
(re-computed locally with `node scripts/check-concept-reviews.mjs --fingerprint
kent-amusements` — matches the repaired candidate recorded in
`CONCEPT_DESIGN_REVIEW.md` → "Re-review candidate — repaired source fingerprints")
...
Current presence and verification: `research/verifications.json` → "Kent
...
- Current capture: `research/verifications.json` — Facebook/Instagram only; no
...
- Sources, asset credits and limitations: `research/verifications.json`;
  `public/images/place/ATTRIBUTION.md` line 11.
...
- `pnpm build`: not re-run — the brief reserved the build to another process. Its
  first gate runs standalone: `node scripts/check-concept-reviews.mjs` → "Concept
...
| Repository and journey checks | Pass | `scripts/test-concept-shell.mjs` 72/72 passed, 0 failed, run serially with no competing agents; `scripts/test-reviewed-concept-journeys.mjs` 14/14 passed; `scripts/check-concept-reviews.mjs` passed and its `--self-test` rejected the invalid fixture with 9 errors; zero console errors on all four route/size combinations. `pnpm build` itself was not re-run because the brief reserved it to another process. |
...
= **6.375**, recorded as **6.38/10** using `scripts/check-concept-reviews.mjs`
...
all six material-claim classes against `research/verifications.json`; computed
