...
(re-computed by the reviewer via `node scripts/check-concept-reviews.mjs --fingerprint mourne-cycles`; matches the repaired candidate recorded in `CONCEPT_DESIGN_REVIEW.md` § "Re-review candidate — repaired source fingerprints")
...
- Hero imagery: AI-generated visualisation, disclosed in a figcaption pinned to
  the image and documented in `public/images/place/ATTRIBUTION.md`.
...
- Sources, asset credits and limitations: `public/images/place/ATTRIBUTION.md`
  lines 14 and 18; `src/data/transformation-details.ts` lines 731 and 744.
...
3. **Fix the three remaining contrast failures, and fix the probe that missed
   ...
   *Acceptance check:* extend `CONTRAST_PROBE` in
   `scripts/test-reviewed-concept-journeys.mjs` to multiply inherited CSS
